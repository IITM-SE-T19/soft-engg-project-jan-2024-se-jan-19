...
class FAQAPI(Resource):
    ...
    # Add a search method to FAQAPI class
    def search(self):
        query = request.args.get('query', '')
        if query:
            faqs = FAQ.search(query)
            return jsonify([faq_util.convert_faq_to_dict(faq) for faq in faqs]), 200
        else:
...
# Register the search method as a route
faq_api.add_resource(FAQAPI, "/", "/search")
