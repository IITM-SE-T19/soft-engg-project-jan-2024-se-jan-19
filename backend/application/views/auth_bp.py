...
#         Usage
#         -----
#         For the user register page. It checks user data and raises appropriate error
#         if required. Creates a user account and generates a user token and returns it.

#         Parameters
#         ----------
#         form data sent with request
#         data format {'first_name':'', 'last_name':'', 'email':'',
#                     'password':'', 'retype_password':'', 'role':'', 'discourse_id':''}
#         'last_name' is optional
...
#             "retype_password": "",
#             "role": "",
#             "discourse_id": "",  # New field for Discourse id
#         }
...
#         except Exception as e:
#             logger.error(
#                 f"Register->post : Error occurred while getting form data : {e}"
#             )
#             raise InternalServerError
#         else:
#             for key in details:
...
#                     raise AlreadyExistError(status_msg="Email is already in use")
#                 else:
#                     # Check if Discourse id is unique
#                     if Auth.query.filter_by(discourse_id=details["discourse_id"]).first():
#                         raise AlreadyExistError(status_msg="Discourse id is already in use")

#                     # Generate unique user_id
#                     user_id = auth_utils.generate_user_id(email=details["email"])

#                     # Create a new user in Auth table
#                     details["user_id"] = user_id
#                     user = auth_utils.update_auth_table(details=details)
